...
All of the examples here show routes which take a shallow-cloned instance of
the web application as a first parameter. This is true even if those routes
...
=head3 Step 0: (Optional) Change the base controllr

It is a good practice to set up a different base, so that you separate general
...
=head3 Step 1: Create a main controller class

This step is done already if you haven't changed the C<base>.
...
=head3 Step 2: Create any number of controller classes
...
=head3 Step 3: Add routes with shorter class names
