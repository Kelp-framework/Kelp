...
Kelp does not implement template layouts by itself, so it's up to templating engine to deliver that behavior. For example, L<Template::Toolkit> allows for C<WRAPPER> directive, which can be used like this (with L<Kelp::Module::Template::Toolkit>):
